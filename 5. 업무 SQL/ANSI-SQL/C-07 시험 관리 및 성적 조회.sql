...
select distinct   e.courseName as "과정명", d.subjectName as "과목명",  a.startdate as "과정 시작일", a.enddate as "과정 종료일", (case when f.question is not null then 'Y' else 'N' end) as "시험문제등록여부" , (case when g.score is not null then 'Y' else 'N' end) as "성적등록여부" 
...
select distinct   e.courseName as "과정명", d.subjectName as "과목명",  a.startdate as "과정 시작일", a.enddate as "과정 종료일" , z.classroomname as "강의실명" , x.name as "교육생 이름" ,  x.jumin as "주민번호 뒷자리" , f.testtype as"유형" , g.score as "성적"
from  tblOpenedCourse a
    inner join tblOpenedSubject b on b.ocseq = a.seq
    inner join tblSubjectCourse c on  b.scseq  = c.seq
    inner join tblSubject d on c.sseq = d.seq
...
where  b.seq = '개설과목seq'
    order by x.name asc, f.testtype desc;
...
select distinct   e.courseName as "과정명", d.subjectName as "과목명",  a.startdate as "과정 시작일", a.enddate as "과정 종료일" , z.classroomname as "강의실명" , x.name as "교육생 이름" ,  x.jumin as "주민번호 뒷자리" , f.testtype as"유형" , g.score as "성적", 출결점수
from  tblOpenedCourse a
    inner join tblOpenedSubject b on b.ocseq = a.seq
    inner join tblSubjectCourse c on  b.scseq  = c.seq
    inner join tblSubject d on c.sseq = d.seq
...
where x.seq = '학생seq'
order by d.subjectName, f.testtype desc;
